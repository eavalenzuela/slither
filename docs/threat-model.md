...
tracepoints (the container collector), + kprobes on the IP send and UDP receive
paths reading UDP/53 payloads (the DNS collector),
...
| **Tampering** | Local attacker modifies the agent's state, rules, or audit chain | **Partially defended.** State dir 0700 root:root (Phase 5 #94). PR_SET_DUMPABLE=0 blocks ptrace + core dumps + /proc/<agent-pid>/* reads from non-owner UIDs. Tamper-evident hash chain (Phase 5 #95) makes silent log edits detectable on next boot. Phase 6 #112 added a server-side count cross-check, and Phase 7 / ADR-0042 upgraded it to record level: the agent ships each chain record's `(seq, prev_hash, record_hash)` tuple and the server keeps them as an append-only witness, so **edit-then-relink** — rewriting a record and re-chaining everything after it, which leaves both the on-disk verify and the count clean — is now caught within one 5-minute summary interval. None of this defends against a local-root attacker who owns the host BEFORE a record is first shipped, or who can write the chain AND control the agent's outbound channel — that's the explicit non-goal. |
...
| **Information disclosure** | Side-channel readout of agent memory | **Partially defended.** PR_SET_DUMPABLE=0 + state-dir lockdown covers /proc and core dumps. Cert + key live in `/etc/slither/` files at 0600 root:root as the durable store. The kernel keyring (Phase 5 #98 + Phase 6 #117 / ADR-0038) holds an in-RAM copy under `KEY_SPEC_USER_KEYRING` (`@u`) — survives the enrol→service session boundary that Gap A originally exposed, available since kernel 3.5, well below the project's 5.15 floor. `@u` is per-uid, not per-process: any process running as the agent's uid (root in the standard deployment) can read the keys via `keyctl`, which reduces to "another root process on the same host" — already out of scope. Operators wanting tighter scoping run the agent under a dedicated uid. Memory of a running process is still visible to a CAP_SYS_PTRACE-holding peer; we don't defend against another root process on the same host. The TPM-sealed variant (Phase 6 #118, opt-in via `agent.keystore.tpm: true`) is the upgrade path for hosts with TPM 2.0 + Secure Boot. |
...
- Cert + key on disk at `/etc/slither/` 0600 root:root (Phase 5 #98
  files-first path); Phase 6 #117 / ADR-0038 picked
  `KEY_SPEC_USER_KEYRING` (`@u`) as the durable in-RAM target so the
  keyring write survives the enrol→service session boundary, with
  #118 layering a TPM-sealed variant for opt-in hosts.

...
14. Phase 1 exit validation on RHEL 9 and Debian 13 (manual). *(RHEL 9 portability fix 2026-04-22: kernel 5.14 rejected the file collector with `bpf(BPF_LINK_CREATE, attach_type=BPF_PERF_EVENT, …) = -1 EACCES` on every `tracepoint/syscalls/sys_enter_*` attach. Root cause: the 5.14 verifier enforces `prog->aux->max_ctx_offset ≤ trace_event_get_offsets(tp)`; per-syscall tracepoints expose a context struct sized as `16 + nb_args*8` while our BPF compiled against the generic `trace_event_raw_sys_enter` with `args[6]` (offset up to 64). sched tracepoints pass because their per-TP struct size matches what the program reads. Fix: consolidate the five file-syscall handlers into a single `tracepoint/raw_syscalls/sys_enter` program that dispatches on `ctx->id` — the generic tracepoint's event context matches the compile-time struct, so `max_ctx_offset` check always passes. Collector attaches once to `raw_syscalls/sys_enter` instead of five per-syscall tracepoints; in-kernel switch dispatches to openat/unlinkat/renameat2/fchmodat/fchownat by amd64 syscall number. All bpf2go-regenerated; -race test suite clean.)* *(Debian 13 complete 2026-04-22: kernel `6.12.74+deb13+1-amd64`, service active under the shipped unit with CAP_BPF/CAP_PERFMON/CAP_SYS_PTRACE/CAP_DAC_READ_SEARCH bounded. OCSF `process_activity` (class_uid 1007) confirmed for `/bin/true` with full actor chain. Real `DetectionFinding` (class_uid 2004, `rule.uid` `8b7c4d00-0001-4000-8000-000000000001` — "Bash reverse shell via /dev/tcp", severity_id 4, `x_triggering_event_ids` linking back to the process event) fired against the shipped reverse-shell scenario. `make load-test` produced `drop_rate_pct=0.09%` (see task #15 for the optimisation work that got us there). Loader required `kernel.perf_event_paranoid=2` (Debian defaults to 3, fix shipped in `deploy/sysctl.d/99-slither.conf` at `87e97fa`). Raw outputs in `debian_13_phase1_validation` at repo root. **RHEL 9 validation still pending.**)*

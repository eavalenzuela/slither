...
14. Phase 1 exit validation on RHEL 9 and Debian 13 (manual).
15. Enricher worker pool — pid-sharded parallel /proc backfill. *(Scoped 2026-04-22 after task #29 Debian 13 load test measured 46.08% drop rate at 11k events/s with `ringbuf_overflow=0` and peak_cpu=6.9% on a 4-core host. Root cause: single-goroutine enricher Run loop serialised three `/proc/<pid>/{status,exe,cmdline}` reads per ProcExec, producing an I/O-bound ceiling of ~6k events/s. Fix landed in three passes:
